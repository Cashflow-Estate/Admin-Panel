...
const PhotosTab = () => {
  ...
  return (
    <Fragment>
      ...
      <PagesSort />
      {/* {photoIndex.isOpen && (
        <Lightbox mainSrc={require(`../../../assets/images/${imagess[photoIndex.index]}`)} nextSrc={require(`../../../assets/images/${imagess[(photoIndex.index + 1) % imagess.length]}`)} prevSrc={require(`../../../assets/images/${imagess[(photoIndex.index + imagess.length - 1) % imagess.length]}`)} imageTitle={photoIndex.index + 1 + "/" + imagess.length} onCloseRequest={() => setPhotoIndex({ ...photoIndex, isOpen: false })} onMovePrevRequest={onMovePrev} onMoveNextRequest={onMoveNext} />
      )} */}
    </Fragment>
  );
};
